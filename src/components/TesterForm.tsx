...
import { LockKey, Envelope, User, CheckCircle, Phone, ChatText } from "@phosphor-icons/react";
import clsx from "clsx";

export default function TesterForm() {
  const [status, setStatus] = useState<"idle" | "loading" | "success" | "error">("idle");
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", feature: "" });
        ...
        >
          ...
          <div className="relative z-10 max-w-xl mx-auto text-center mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-bold tracking-tighter text-zinc-900 mb-4">
              Trở thành <span className="text-emerald-600">Beta Tester.</span>
            </h2>
            <p className="text-[16px] text-zinc-500 leading-relaxed md:max-w-md mx-auto">
              Hãy là những người đầu tiên trải nghiệm tính năng mới của UNIBEAN và đóng góp ý kiến để hoàn thiện hệ thống.
            </p>
          </div>
          ...
          <div className="relative w-full max-w-lg mx-auto">
            {status === "success" ? (
              ...
              >
                ...
                <h3 className="text-2xl font-bold text-zinc-900 mb-2">Truy cập hợp lệ</h3>
                <p className="text-zinc-500 max-w-[30ch]">
                  Chúng tôi đã nhận được thông số của bạn. Hãy kiểm tra hòm thư để nhận hướng dẫn Sandbox.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="name" className="text-[13px] font-bold text-zinc-600 uppercase tracking-widest pl-1">
                    Họ và tên *
                  </label>
                  <div className="relative">
                    <User weight="bold" className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 text-lg" />
                    <input
                      id="name"
                      type="text"
                      required
                ...
                {/* Custom Input 2 */}
                <div className="flex flex-col gap-2">
                  <label htmlFor="email" className="text-[13px] font-bold text-zinc-600 uppercase tracking-widest pl-1">
                    Email *
                  </label>
                  <div className="relative">
                    <Envelope weight="bold" className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 text-lg" />
                    <input
                      id="email"
                      type="email"
                      required
                ...
                {/* Custom Input 3 */}
                <div className="flex flex-col gap-2">
                  <label htmlFor="phone" className="text-[13px] font-bold text-zinc-600 uppercase tracking-widest pl-1">
                    Số Điện Thoại *
                  </label>
                  <div className="relative">
                    <Phone weight="bold" className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 text-lg" />
                    <input
                      id="phone"
                      type="tel"
                      required
                ...
                {/* Custom Input 4 */}
                <div className="flex flex-col gap-2">
                  <label htmlFor="feature" className="text-[13px] font-bold text-zinc-600 uppercase tracking-widest pl-1">
                    Bạn muốn trải nghiệm tính năng nào nhất ?
                  </label>
                  <div className="relative">
                    <ChatText weight="bold" className="absolute left-4 top-4 text-zinc-400 text-lg" />
                    <textarea
                      id="feature"
                ...
                <motion.button
                  ...
                  className={clsx(
                    ...
                    status === "loading" ? "bg-emerald-700 opacity-80 cursor-wait shadow-none" : "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/20"
                  )}
                >
                  {status === "loading" ? (
                    <motion.div 
